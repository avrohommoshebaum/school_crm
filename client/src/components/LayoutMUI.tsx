...
interface NavChildItem {
  ...
  path: string;
  icon: IconType;
  permission?: { module: string; action: "view" };
}
...
export default function LayoutMUI() {
  ...
  const rolesArray = Array.isArray((user as any)?.roles) ? ((user as any).roles as any[]) : [];
  const { can } = usePermissions({ roles: rolesArray as any });
  ...
  // If user is null, we’re redirecting (show something instead of blank)
  if (!user) {
    return (
      <Box sx={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        Redirecting…
  ...
  const handleProfileMenuOpen = (event: MouseEvent<HTMLElement>) => setProfileMenuAnchor(event.currentTarget);
  ...
  const canSeeItem = (item: NavItem) => !item.permission || can(item.permission.module, item.permission.action);

  const isActive = (path: string) => location.pathname === path;
  const isParentActive = (item: NavItem) => isActive(item.path) || item.children?.some((child) => isActive(child.path));
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      ...
      <Drawer
        ...
        sx={{
          ...
          flexShrink: 0,
          "& .MuiDrawer-paper": {
            width: isMobile ? DRAWER_WIDTH_OPEN : drawerOpen ? DRAWER_WIDTH_OPEN : DRAWER_WIDTH_CLOSED,
          ...
          {visibleNavItems.map((item) => {
              ...
              return (
                <Box key={item.name}>
                  ...
                  >
                    ...
                    {showText && <ListItemText primary={item.name} primaryTypographyProps={{ fontSize: "0.875rem" }} />}
                        ...
                        return (
                          ...
                          >
                            <ListItemIcon sx={{ minWidth: 40 }}>
                              <child.icon sx={{ fontSize: 20, color: activeChild ? "#1976d2" : "inherit" }} />
                            </ListItemIcon>
                            <ListItemText primary={child.name} primaryTypographyProps={{ fontSize: "0.8125rem" }} />
                          </ListItemButton>
                        );
                      })}
                    </List>
            ...
            return (
              ...
              >
                ...
                {(drawerOpen || isMobile) && <ListItemText primary={item.name} primaryTypographyProps={{ fontSize: "0.875rem" }} />}
              </ListItemButton>
            );
          })}
        </List>

        {/* ✅ Bottom user info (UNCHANGED behavior: only shows when drawerOpen) */}
        {drawerOpen && user && (
          <Box sx={{ mt: "auto", p: 2, borderTop: "1px solid #e0e0e0" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <Avatar sx={{ width: 40, height: 40, bgcolor: "#1976d2", fontWeight: "bold" }}>
                {user.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  {(user.roles || [])
                    .map((r: any) => (typeof r === "string" ? r : r.displayName || r.name || ""))
                    .filter(Boolean)
                    .join(", ") || "—"}
                </Typography>
              </Box>
